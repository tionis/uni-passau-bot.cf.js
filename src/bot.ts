...
async function getPrettyMensaInfoForWeek(
	env: Env,
	weekNumber: number,
): Promise<string> {
	...
	let dates = data.map((entry) => entry.date.toISOString().split("T")[0])
		.filter((value, index, self) => self.indexOf(value) === index).sort();
	for (const date of dates) {
		const entries = data.filter((entry) =>
			entry.date.toISOString().split("T")[0] === date
		);
		const weekday = entries[0].date.toLocaleDateString("de-DE", {
			weekday: "long",
		});
		prettyData += `\n*${weekday} (${
			entries[0].date.toLocaleDateString("de-DE")
		})*:\n`;
		...
		switch (msg.text) {
			case "/start":
				await tgSendMessage(
					env,
					msg.chat.id,
					"Hello! I'm a unofficial Bot for the University of Passau. How can I help you today?",
				);
				break;
			case "/mensa":
			case "/mensatoday":
			...
			case "Food for tomorrow":
				...
				const mensaInfoTomorrow = await getPrettyMensaInfoForDate(
					env,
					tomorrow,
				);
			...
			case "Food for next week":
				...
				const mensaInfoNextWeek = await getPrettyMensaInfoForWeek(
					env,
					nextWeekNumber,
				);
					...
					case "/contact":
						...
						if (messageForDev === "") {
							await tgSendMessage(
								env,
								msg.chat.id,
								"Please provide a message to send to the developer after the command",
							);
							break;
						}
						if (msg.from) {
							...
							await tgSendMessage(
								env,
								msg.chat.id,
								"Unable to send message to developer. User information is missing.",
							);
						}
						break;
					default:
...
export { getPrettyMensaInfoForDate, getPrettyMensaInfoForWeek, handleMessage };
